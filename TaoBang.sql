...
CREATE TABLE PHIEU_THUE (
    ...
    -- Tạo khóa ngoại tham chiếu đến bảng HOP_DONG
    CONSTRAINT FK_PHIEUTHUE_HOPDONG 
        FOREIGN KEY (MAHOPDONG) REFERENCES HOP_DONG(MAHOPDONG)
);

-- Tao bang ky thanh toán 
CREATE TABLE KY_THANH_TOAN (
    MAKYTHANHTOAN VARCHAR(10) PRIMARY KEY,
    MAHOPDONG VARCHAR(10) NOT NULL,
    THANGTHU INT NOT NULL CHECK (THANGTHU BETWEEN 1 AND 12),
    NAMTHU INT NOT NULL CHECK (NAMTHU >= 2024),
    TIENPHAITHRA DECIMAL(12,2) NOT NULL CHECK (TIENPHAITHRA > 0),
    TONGDATHU DECIMAL(12,2) DEFAULT 0 CHECK (TONGDATHU >= 0),
    TINHTRANG NVARCHAR(20) NOT NULL CHECK (TINHTRANG IN (N'Chưa thu', N'Thu một phần', N'Đã thu đủ')),
    
    -- Tạo khóa ngoại tham chiếu đến bảng HOP_DONG
    CONSTRAINT FK_KYTHANHTOAN_HOPDONG 
        FOREIGN KEY (MAHOPDONG) REFERENCES HOP_DONG(MAHOPDONG),
    
    -- Ràng buộc unique cho một hợp đồng trong một tháng/năm
    CONSTRAINT UK_HOPDONG_THANG_NAM UNIQUE (MAHOPDONG, THANGTHU, NAMTHU)
);
...
CREATE TABLE PHIEU_THU (
    ...
    MACH VARCHAR(10) NOT NULL,
    MANV_DITHU VARCHAR(10) NOT NULL,
    MAKYTHANHTOAN VARCHAR(10) NOT NULL,
    NGAYLAPPHIEU DATE NOT NULL,
    TRANGTHAITHU NVARCHAR(20) NOT NULL CHECK (TRANGTHAITHU IN (N'Thu đủ', N'Không đủ')),
    
    -- Tạo khóa ngoại
    CONSTRAINT FK_PHIEUTHU_CANHO 
        FOREIGN KEY (MACH) REFERENCES CAN_HO(MACH),
    
    CONSTRAINT FK_PHIEUTHU_NHANVIEN 
        FOREIGN KEY (MANV_DITHU) REFERENCES NHAN_VIEN(MANV),
    
    CONSTRAINT FK_PHIEUTHU_KYTHANHTOAN 
        FOREIGN KEY (MAKYTHANHTOAN) REFERENCES KY_THANH_TOAN(MAKYTHANHTOAN)
);


-- Tạo bảng chi tiết thanh toán 
CREATE TABLE CHI_TIET_THANH_TOAN (
    MACHITIETTHANHTOAN VARCHAR(10) PRIMARY KEY,
    MAPHIEUTHU VARCHAR(10) NOT NULL,
    NGAYTHU DATE NOT NULL,
    MANV_THU VARCHAR(10) NOT NULL,
    SOTIENTHU DECIMAL(12,2) NOT NULL CHECK (SOTIENTHU > 0),
    
    -- Tạo khóa ngoại
    CONSTRAINT FK_CHITIET_PHIEUTHU 
        FOREIGN KEY (MAPHIEUTHU) REFERENCES PHIEU_THU(MAPHIEUTHU),
    
    CONSTRAINT FK_CHITIET_NHANVIEN 
        FOREIGN KEY (MANV_THU) REFERENCES NHAN_VIEN(MANV)
);